...
local function setup_integration_mocks()
	...
	return {
		...
		get_last_extmark = function() return extmark_calls[#extmark_calls] end
	}
end
...
async.tests.describe("turbo-needle ghost integration", function()
	...
	async.tests.it("should display and accept a mocked single-line completion", async.void(function()
		turbo_needle.setup()
	...
	async.tests.it("should display and accept a mocked multi-line completion", async.void(function()
		turbo_needle.setup()
		...
		local prefix = table.concat({
			original_lines[1], original_lines[2], original_lines[3],
			"    total = total"
		}, "\n")
		local suffix = " + v\n  end\n  return total\nend"
